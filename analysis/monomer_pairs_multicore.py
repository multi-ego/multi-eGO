...
extended_ai, extended_aj, monomer_distances, list_monomer_pairs_df = [], [], [], []
...
# exclusion list degli aminoacidi
    ...
    print(f'At frame nr {frame.frame} out of {total_frames} there are {len(monomer_pairs_df)} pairs', end='\r')
    list_monomer_pairs_df.append(monomer_pairs_df)

print('len traj', len(u.trajectory))
print('len_list', len(list_monomer_pairs_df))

monomer_pairs_df = pd.concat(list_monomer_pairs_df)
...
for pair, n_counter in enumerate(pairs_list, 1):

    print(f'Processing {n_counter} out of {total_pairs}: {pair}', end='\r')
...
file = open('monomer_pairs_multicore.txt', 'w')
