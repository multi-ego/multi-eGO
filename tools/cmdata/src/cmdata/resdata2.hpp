...
namespace cmdata
{
  ...
  static void count_same(
    const int i, const int mol_i, const std::size_t a_i, const std::size_t a_j, const std::vector<int> &natmol2,
    const std::vector<int> &mol_id, std::vector<std::vector<std::mutex>> &frame_same_mutex_,
    std::vector<std::vector<int>> &frame_same_mat_)
  {
    std::size_t same_access_index = cmdata::indexing::offset_same(mol_id[i], mol_i, a_i, a_j, natmol2);
    std::size_t same_mutex_index = cmdata::indexing::mutex_access(mol_id[i], a_i, a_j, natmol2);
    std::unique_lock lock(frame_same_mutex_[mol_id[i]][same_mutex_index]);
    frame_same_mat_[mol_id[i]][same_access_index]++;
    lock.unlock();
  }

  static void count_cross(
    const int i, const int j, const int mol_i, const int mol_j, const std::size_t a_i, const std::size_t a_j, const std::vector<int> &natmol2,
    const std::vector<std::vector<int>> &cross_index_, const std::vector<int> &mol_id, std::vector<std::vector<std::mutex>> &frame_cross_mutex_,
    std::vector<std::vector<int>> &frame_cross_mat_)
  {
    std::size_t cross_access_index = cmdata::indexing::offset_cross(mol_id[i], mol_id[j], mol_i, mol_j, a_i, a_j, natmol2);
    std::size_t cross_mutex_index = cmdata::indexing::mutex_access(mol_id[j], a_i, a_j, natmol2);
    std::unique_lock lock(frame_cross_mutex_[cross_index_[mol_id[i]][mol_id[j]]][cross_mutex_index]);
    frame_cross_mat_[cross_index_[mol_id[i]][mol_id[j]]][cross_access_index]++;
    lock.unlock();
  }
class CMData
{
private:
  ...
  // resdata probabilities
  std::vector<std::vector<std::vector<double>>> interm_same_prob_;
  std::vector<std::vector<std::vector<double>>> interm_cross_prob_;
  ...
  // function types
  // using ftype_intra_ = cmdata::ftypes::function_traits<decltype(&cmdata)>;
  using ftype_same_ = cmdata::ftypes::function_traits<decltype(&count_same)>;
  using ftype_cross_ = cmdata::ftypes::function_traits<decltype(&count_cross)>;
  ...
  static void molecule_routine(
    ...
    const std::vector<std::vector<int>> &cross_index_, const double mcut2_, 
    rvec *xcm_, const gmx::RangePartitioning &mols_, gmx_mtop_t *mtop_, 
    std::vector<std::vector<int>> &frame_same_mat_, std::vector<std::vector<std::mutex>> &frame_same_mutex_,
    cmdata_matrix &interm_same_mat_density_, std::vector<std::vector<int>> &frame_cross_mat_,
    std::vector<std::vector<std::mutex>> &frame_cross_mutex_, cmdata_matrix &interm_cross_mat_density_,
    cmdata::parallel::Semaphore &semaphore_, const std::function<ftype_same_::signature> &f_inter_mol_same_,
  ...
  */
  {
    ...
    if (same_) frame_same_mutex_.resize(natmol2_.size());
  ...
  void calc_probabilities(
    const cmdata::indexing::SameThreadIndices &same_indices, const cmdata::indexing::CrossThreadIndices &cross_indices,
    const std::vector<int> &natmol2, const std::vector<int> &nresmol2, const std::vector<std::vector<int>> &res_ndx, 
    const std::vector<std::vector<int>> &res_sizes, const std::vector<std::vector<int>> &num_mol_unique,
    std::vector<std::vector<int>> &frame_same_mat, const std::vector<std::vector<int>> &cross_index,
    std::vector<std::vector<int>> &frame_cross_mat, cmdata_matrix &interm_same_mat_density,
    cmdata_matrix &interm_cross_mat_density
  )
  {
    for ( std::size_t i = 0; i < natmol2.size(); i++ )
    {
      for ( std::size_t a_i = 0; a_i < natmol2[i]; a_i++ )
      {
        for ( std::size_t a_j = 0; a_j < natmol2[i]; a_j++ )
        {
    ...
    while (frame_->read_next_frame(trj_, no_pbc_, pbcType_, pbc_) == exdrOK)
    {
          ...
          ( dt_ == 0 || std::fmod(frame_->time, dt_) == 0) && (nskip_ == 0 || std::fmod(frnr, nskip_) == 0)) // skip frames
      {
        ...
        for (int i = 0; i < nindex_; i++)
        {
          ...
          molecule_routine(i, nindex_, pbc_, frame_->x, inv_num_mol_, cut_sig_2_, natmol2_, num_mol_unique_, mol_id_, cross_index_,
            mcut2_, xcm_, mols_, mtop_, frame_same_mat_, frame_same_mutex_, interm_same_mat_density_, frame_cross_mat_,
            frame_cross_mutex_, interm_cross_mat_density_, semaphore_, f_inter_mol_same_, f_inter_mol_cross_, 1.0);
          /* end molecule thread */
        }
        ...
        for ( int tid = 0; tid < num_threads_; tid++ )
        {
          // threads_[tid] = std::thread(
            ...
            // std::cref(frame_same_mat_), std::cref(cross_index_), 
