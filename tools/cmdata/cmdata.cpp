...
void CMData::initAnalysis(const TrajectoryAnalysisSettings& settings, const TopologyInformation& top)
{
    ...
    for (int i = 0; i < density_bins_.size(); i++ ) density_bins_[i] = cutoff_/static_cast<double>(density_bins_.size())*static_cast<double>(i)+cutoff_/static_cast<double>(density_bins_.size()*2);
    ...
    for(std::size_t i=0; i<natmol2_.size(); i++) {
       for(int ii=0; ii<natmol2_[i]; ii++) {
          for(int jj=ii; jj<natmol2_[i]; jj++) {
             std::transform(interm_same_mat_density_[i][ii][jj].begin(), interm_same_mat_density_[i][ii][jj].end(), interm_same_mat_density_[i][ii][jj].begin(), [&norm](auto& c){return c*norm;});
             std::transform(intram_mat_density_[i][ii][jj].begin(), intram_mat_density_[i][ii][jj].end(), intram_mat_density_[i][ii][jj].begin(), [&norm](auto& c){return c*norm;});
       ...
       for(std::size_t j=i+1; j<natmol2_.size(); j++) {
          for(int ii=0; ii<natmol2_[i]; ii++) {
             for(int jj=0; jj<natmol2_[j]; jj++) {
                std::transform(interm_cross_mat_density_[cross_index_[i][j]][ii][jj].begin(), interm_cross_mat_density_[cross_index_[i][j]][ii][jj].end(), interm_cross_mat_density_[cross_index_[i][j]][ii][jj].begin(), [&norm](auto& c){return c*norm;});
             }
          }
       }
    }
