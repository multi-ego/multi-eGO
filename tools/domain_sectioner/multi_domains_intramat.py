...
if __name__ == "__main__":
    ...
    if args.type == "group" and args.mego_top is None:
...
if args.type == "split":
    ...
    intra_md[4] = np.where(domain_mask_linear, intra_md[4], 0.0)
    intra_md[5] = np.where(domain_mask_linear, intra_md[5], 0.0)
...
if args.type == "group":
    ...
    np.savetxt(
        f'{args.out}group_{"-".join(np.array(args.dom_res, dtype=str))}_{intra1}',
        intra_rc.T,
        delimiter=" ",
        fmt=["%i", "%i", "%i", "%i", "%2.6f", "%.6e", "%2.6f", "%1i"],
    )
